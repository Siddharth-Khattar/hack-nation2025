...
import { getMarkets } from "@/lib/api/endpoints/markets";
import { getGraphVisualization } from "@/lib/api/endpoints/relations";
import {
  transformApiGraphToGraphData,
  validateGraphData,
} from "@/lib/transforms/graphApiTransform";
...
const DEFAULT_OPTIONS: Required<UseGraphDataOptions> = {
  ...
  maxRelationsPerNode: parseInt(
    process.env.NEXT_PUBLIC_MAX_RELATIONS_PER_NODE || "10",
    10
  ),
  enableCache: true,
...
export function useGraphData(options?: UseGraphDataOptions): UseGraphDataState {
  ...
  const fetchGraphData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch markets with full details (including outcomes and prices)
      console.log(
        `Fetching top ${config.nodeLimit} markets with full details...`
      );
      ...
      // Fetch graph visualization data (nodes and connections)
      console.log("Fetching graph visualization data...");
      const graphResponse = await getGraphVisualization({
        limit: config.nodeLimit,
      ...
      // Transform and merge data from both endpoints
      const graphData = transformApiGraphToGraphData(
